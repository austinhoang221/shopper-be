...
{
    public Expression<Func<TEntity, TResponse>>? Mapping { get; private set; } = default;

    public bool UseDistinct { get; private set; }

    internal static new SpecificationWithMapping<TEntity, TResponse> New() => new();
    ...
    where TEntity : class, IEntityBase
{
    ...
    //Filters
    public IFilter<TEntity>? Filter { get; private set; } = default;
    public List<Expression<Func<TEntity, bool>>> FilterExpressions { get; } = [];

    //SortBy
    public ISortBy<TEntity>? SortBy { get; private set; } = default;
    ...
    public Func<IQueryable<TEntity>, IQueryable<TEntity>>? IncludeAction { get; private set; } = default;
